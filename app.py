...
import edge_tts
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM, pipeline, AutoModelForTextToWaveform
...
def transcribe(audio_path, log_func): # Renamed 'log' to 'log_func' to avoid confusion with the async log
    log_func("[INFO] Transcribing English audio...")
    audio, sr = sf.read(audio_path)
    result = asr_pipeline({"array": audio, "sampling_rate": sr})
    return result['text']

def translate_to_telugu(text, log_func): # Renamed 'log' to 'log_func'
...
def synthesize_telugu(text, output_file, log_func): # Renamed 'log' to 'log_func'
...
async def async_log_generator(queue):
    """Helper to yield logs from a queue."""
    while True:
        msg = await queue.get()
        if msg is None:
            break
...
def run_pipeline():
    ...
    def event_stream():
        ...
        # Consume messages synchronously for Flask Response
        while True:
...
if __name__ == "__main__":
    ...
    app.run(host="127.0.0.1", port=5000, debug=True, threaded=True) # Enabled threading for development
